...
4. 同一轮也保留原有长期记忆 auto recall；只有实际注入 experience 时，才把 experience 和长期记忆分成不同 section。
5. 有 experience section 时，用统一的 `<openviking-context>` 外壳注入到 latest user message 前；没有 experience section 时，长期记忆继续使用旧 `<relevant-memories>` 外壳。
...
examples/openclaw-plugin/context-engine.ts
  assemble()
    ...
    -> if experience hit: buildOpenVikingContextBlock()
    -> else: keep legacy <relevant-memories>
...
- `buildAutoRecallContext()` 过滤掉 experience memory，避免 experience 出现在长期记忆区；同时保留旧 `<relevant-memories>` block，供没有 experience 命中时保持兼容行为。
...
然后按是否真的注入 experience 决定外壳：

```ts
const combinedBlock = experienceRecall.block
  ? buildOpenVikingContextBlock({
      sections: [experienceRecall.block, recall.section ?? recall.block],
    })
  : (recall.block ?? "");
```

如果没有 experience 命中，普通长期记忆仍然是旧 `<relevant-memories>` 格式。只有 experience 命中时，才使用 `<openviking-context>`。如果两个结果都为空，直接 passthrough，不改写 messages。
...
只有实际注入 agent experience 时，才使用新格式：
...
- 没有 experience section 时继续生成旧 `<relevant-memories>`，确保默认关闭 experience 后，普通 auto recall 的 prompt 形态不变。
- 清理逻辑兼容 `<relevant-memories>`、`<openviking-context>` 和旧 `<openviking-agent-experiences>`。
...
- 默认关闭 experience 时，普通长期记忆 auto recall 仍使用旧 `<relevant-memories>` 外壳。
